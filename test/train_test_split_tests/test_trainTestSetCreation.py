...
# ------- remove_edges_with_nodes --------------------
    def test_remove_edges_with_nodes_none_removed(self):
        #given
        edges = pd.DataFrame({globalConst.NODE1_ID_COL_NAME: ['xyz','foo','bar'],
        ...
        #then
        true_result = pd.DataFrame({globalConst.NODE1_ID_COL_NAME: ['xyz', 'foo', 'bar'],
                                  globalConst.EDGE_TYPE_COL_NAME: list('abc'),
                                  globalConst.NODE2_ID_COL_NAME: ['foo', 'bar', 'baz']})
    ...
    def test_remove_edges_with_nodes_removed_some(self):
        ...
        result = TrainTestSetCreation.remove_edges_with_nodes(samples=edges, nodes=nodes)
        #then
        true_result = pd.DataFrame({globalConst.NODE1_ID_COL_NAME: ['bar'],
    ...
    def test_remove_edges_with_nodes_remove_all(self):
        ...
        result = TrainTestSetCreation.remove_edges_with_nodes(samples=edges, nodes=nodes)
        #then
        true_result = pd.DataFrame(columns=[globalConst.NODE1_ID_COL_NAME,
...
# ------- get_additional_nodes --------------------
    def test_get_additional_nodes_none(self):
        ...
        #when
        result = TrainTestSetCreation.get_additional_nodes(old_nodes_list=old_nodes_list,
...
# ------- create_cross_val --------------------
    def test_create_cross_val_folds(self):
        ...
        #given
        edges = pd.DataFrame({globalConst.NODE1_ID_COL_NAME: list('abc'),
                              globalConst.EDGE_TYPE_COL_NAME: list('opq'),
                              globalConst.NODE2_ID_COL_NAME: list('xyz')})
        n = 3
        #when
        result = TrainTestSetCreation.create_cross_val(df = edges,
                                              n_folds=n)
        #then
        first = pd.DataFrame({globalConst.NODE1_ID_COL_NAME: list('a'),
                              globalConst.EDGE_TYPE_COL_NAME: list('o'),
                              globalConst.NODE2_ID_COL_NAME: list('x')})
        second = pd.DataFrame({globalConst.NODE1_ID_COL_NAME: list('b'),
                              globalConst.EDGE_TYPE_COL_NAME: list('p'),
                              globalConst.NODE2_ID_COL_NAME: list('y')})
        third = pd.DataFrame({globalConst.NODE1_ID_COL_NAME: list('c'),
                              globalConst.EDGE_TYPE_COL_NAME: list('q'),
                              globalConst.NODE2_ID_COL_NAME: list('z')})
        true_result = [(first.append(third), second),
                       (second.append(third), first),
...
# ------- time_slice_split --------------------
    def test_time_slice_split_additional_nodes(self):
        ...
        true_result_test = pd.DataFrame({
            globalConst.NODE1_ID_COL_NAME: ['DRUG_55','DRUG_85','GENE_10','GENE_1','GENE_2','GENE_2','GENE_2','GENE_2','GO_GO:0000001','GO_GO:0000001','GO_GO:0000003','PHENOTYPE_HP:0000002'],
            globalConst.EDGE_TYPE_COL_NAME: ['DRUG_BINDING_GENE','DRUG_PHENOTYPE','GENE_BINDACT_GENE','GENE_DIS','GENE_EXPRESSED_ANATOMY','GENE_OVEREXPRESSED_ANATOMY','GENE_PATHWAY','GENE_PATHWAY','IS_A','IS_A','PART_OF','IS_A'],
            globalConst.NODE2_ID_COL_NAME: ['GENE_1','PHENOTYPE_HP:0000005','GENE_1','DIS_DOID:0000002','ANATOMY_UBERON:0000001','ANATOMY_UBERON:0000001','PATHWAY_KEGG:hsa04000','PATHWAY_REACT:R-HSA-1000008','GO_GO:0000002','GO_GO:0000003','GO_GO:0000006','PHENOTYPE_HP:0000005'],
            globalConst.QSCORE_COL_NAME: ['191','','309','0.24587142988000799','gold quality','high quality','','','','','','']
        })
