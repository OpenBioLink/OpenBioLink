...
class GraphWriter:
    ...
    def output_graph_props(self):
        ...
        with open(
            os.path.join(self.graph_dir_path, "graph_props.json"), "w"
        ) as json_file:
    ...
    def output_graph(
        ...
        if one_file_sep is not None:
            GraphWriter().output_graph_in_single_file(
                prefix=prefix,
                file_sep=one_file_sep,
                nodes_dic=nodes_dic,
                edges_dic=edges_dic,
                qscore=print_qscore,
            )
        ...
        if multi_file_sep is not None:
            GraphWriter().output_graph_in_multi_files(
                prefix, multi_file_sep, nodes_dic, edges_dic, qscore=print_qscore
            )
        # lists of all nodes and metaedges
        if node_edge_list:
            GraphWriter().write_node_and_edge_list(
                prefix, nodes_dic.keys(), edges_dic.keys()
            )
    ...
    def output_graph_in_single_file(
        self, prefix, file_sep, nodes_dic, edges_dic, qscore
    ):
        if nodes_dic is not None:
            with open(
                os.path.join(
                    self.graph_dir_path, prefix + gcConst.NODES_FILE_PREFIX + ".csv"
                ),
                "w",
            ) as out_file:
        ...
        if edges_dic is not None:
            with open(
                os.path.join(
                    self.graph_dir_path, prefix + gcConst.EDGES_FILE_PREFIX + ".csv"
                ),
                "w",
            ) as out_file:
                writer = csv.writer(out_file, delimiter=file_sep, lineterminator="\n")
    ...
    def output_graph_in_multi_files(
        self, prefix, file_sep, nodes_dic, edges_dic, qscore
    ):
        # write nodes
        for key, value in nodes_dic.items():
            with open(
                os.path.join(
                    self.graph_dir_path,
                    prefix + gcConst.NODES_FILE_PREFIX + "_" + key + ".csv",
                ),
                "w",
            ) as out_file:
                writer = csv.writer(out_file, delimiter=file_sep, lineterminator="\n")
        ...
        for key, value in edges_dic.items():
            with open(
                os.path.join(
                    self.graph_dir_path,
                    prefix + gcConst.EDGES_FILE_PREFIX + "_" + key + ".csv",
                ),
                "w",
            ) as out_file:
                writer = csv.writer(out_file, delimiter=file_sep, lineterminator="\n")
    ...
    def write_node_and_edge_list(self, prefix, nodes_list, edges_list):
        with open(
            os.path.join(self.graph_dir_path, prefix + "nodes_list.csv"), "w"
        ) as out_file:
            out_file.writelines(list("\n".join(nodes_list)))

        with open(
            os.path.join(self.graph_dir_path, prefix + "edges_list.csv"), "w"
        ) as out_file:
            out_file.writelines(list("\n".join(edges_list)))
