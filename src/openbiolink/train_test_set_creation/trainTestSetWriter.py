...
class TrainTestSetWriter:
    ...
    def print_sets(
        ...
        test_set[globConst.COL_NAMES_SAMPLES].to_csv(
            os.path.join(self.folder_path, ttsConst.TEST_FILE_NAME),
            sep="\t",
            index=False,
            header=False,
        )
        ...
        train_val_nodes_df["nodeType"] = [
            x[0] for x in train_val_nodes_df["id"].str.split("_")
        ]
        train_val_nodes_df.to_csv(
            os.path.join(self.folder_path, ttsConst.TRAIN_VAL_NODES_FILE_NAME),
            sep="\t",
            index=False,
            header=False,
        )
        ...
        test_nodes_df.to_csv(
            os.path.join(self.folder_path, ttsConst.TEST_NODES_FILE_NAME),
            sep="\t",
            index=False,
            header=False,
        )
        ...
        if new_test_nodes:
            with open(
                os.path.join(self.folder_path, ttsConst.NEW_TEST_NODES_FILE_NAME),
                "w",
                newline="\n",
            ) as file:
        ...
        if num_folds > 1:
            ...
            fill_train_set[globConst.COL_NAMES_SAMPLES].to_csv(
                os.path.join(self.folder_path, ttsConst.TRAIN_FILE_NAME),
                sep="\t",
                index=False,
                header=False,
            )
            self.folder_path = os.path.join(
                self.folder_path, ttsConst.CROSS_VAL_FOLDER_NAME
            )

        i = 0
        for (train_set, val_set), new_val_nodes_for_fold in zip(
            train_val_set_tuples, new_val_nodes
        ):
            if num_folds > 1:
                fold_folder_path = os.path.join(
                    self.folder_path, ttsConst.FOLD_FOLDER_PREFIX + str(i)
                )
            else:
            ...
            if not train_set.empty:
                train_set[globConst.COL_NAMES_SAMPLES].to_csv(
                    os.path.join(fold_folder_path, ttsConst.TRAIN_FILE_NAME),
                    sep="\t",
                    index=False,
                    header=False,
                )
            if not val_set.empty:
                val_set[globConst.COL_NAMES_SAMPLES].to_csv(
                    os.path.join(fold_folder_path, ttsConst.VAL_FILE_NAME),
                    sep="\t",
                    index=False,
                    header=False,
                )
            if new_val_nodes_for_fold:
                with open(
                    os.path.join(fold_folder_path, ttsConst.NEW_VAL_NODES_FILE_NAME),
                    "w",
                    newline="\n",
                ) as file:
    ...
    def print_vanished_edges(self, vanished_edges):
        ...
        vanished_edges.to_csv(
            os.path.join(self.folder_path, ttsConst.VANISHED_FILE_NAME),
            sep="\t",
            index=False,
            header=False,
        )
