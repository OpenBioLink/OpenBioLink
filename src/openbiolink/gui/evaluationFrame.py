...
class EvalFrame(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.title = tk.Label(
            titles_panel, text="(3) Testing and Evaluation", font=controller.title_font
        )

        self.actions_el = self._create_action_el(self)

        threshold_metrics = [
            ThresholdMetricType[item]
            for item in dir(ThresholdMetricType)
        ...
        self.node_or_corrupted_file_el = self._create_nodes_or_corrupted_path_el(
            paths_box
        )
        ...
        ttk.Separator(self.metrics_frame, orient="horizontal").pack(
            side="top", fill="x", pady=10, padx=10, anchor="s"
        )
        for metric in threshold_metrics:
            ...
            cb = tk.Checkbutton(
                self.metrics_frame,
                text=metric.value,
                variable=self.threshold_metrics_dict[metric],
            )
        ...
        ttk.Separator(self, orient="horizontal").pack(
            side="top", fill="x", pady=(15, 1), padx=10, anchor="s"
        )
    ...
    def _create_action_el(self, parent):
        ...
        train_box = tk.Checkbutton(
            el,
            text="perform training",
            variable=self.train,
            command=self.pack_file_paths,
        )
        self.evaluate = tk.BooleanVar(value=True)
        eval_box = tk.Checkbutton(
            el,
            text="perform evaluation",
            variable=self.evaluate,
            command=self.pack_file_paths,
        )
        models = [
            ModelTypes[item].name
            for item in dir(ModelTypes)
            if not item.startswith("__")
        ]
    ...
    def _create_hits_at_k_el(self, parent):
        ...
        self.rank_metrics_dict[RankMetricType.HITS_AT_K_UNFILTERED] = tk.BooleanVar(
            value=True
        )
        hits_cb = tk.Checkbutton(
            panel, text="hits@K", variable=self.hits, command=self.toggl_ranked_metrics
        )
    ...
    def _create_mrr_el(self, parent):
        ...
        self.rank_metrics_dict[RankMetricType.MRR_UNFILTERED] = tk.BooleanVar(
            value=True
        )
        mrr_cb = tk.Checkbutton(
            panel, text="MRR", variable=self.mrr, command=self.toggl_ranked_metrics
        )
        mrr_filtered_cb = tk.Checkbutton(
            panel, text="filtered", variable=self.rank_metrics_dict[RankMetricType.MRR]
        )
    ...
    def _create_config_path_el(self, parent):
        ...
        button = tk.Button(
            panel, text="select path ...", command=lambda: self.browse_config_file()
        )
        label = tk.Entry(el, textvariable=self.config_path)
        panel.pack(side="top", fill="x")
        tk.Label(panel, text="model config path:").pack(
    ...
    def _create_train_path_el(self, parent):
        ...
        train_button = tk.Button(
            panel, text="select path ...", command=lambda: self.browse_train_file()
        )
        train_label = tk.Entry(el, textvariable=self.train_path)
        panel.pack(side="top", fill="x")
        tk.Label(panel, text="training set path:").pack(
            side="left", anchor="w", padx=5, pady=5
        )
    ...
    def _create_test_path_el(self, parent):
        ...
        test_button = tk.Button(
            panel, text="select path ...", command=lambda: self.browse_test_file()
        )
        test_label = tk.Entry(el, textvariable=self.test_path)
        panel.pack(side="top", fill="x")
        tk.Label(panel, text="test set path:").pack(
            side="left", anchor="w", padx=5, pady=5
        )
    ...
    def _create_trained_model_path_el(self, parent):
        ...
        button = tk.Button(
            panel, text="select path ...", command=lambda: self.browse_trained_model()
        )
        label = tk.Entry(el, textvariable=self.trained_model_path)
        panel.pack(side="top", fill="x")
        tk.Label(panel, text="trained model path:").pack(
            side="left", anchor="w", padx=5, pady=5
        )
        button.pack(side="right", anchor="w", padx=5, pady=5)
        label.pack(fill="x", padx=5, pady=5)
    ...
    def _create_nodes_or_corrupted_path_el(self, parent):
        ...
        button = tk.Button(
            panel,
            text="select path ...",
            command=lambda: self.browse_nodes_or_corr_file(),
        )
        self.select_choices_node_corrupted = ["nodes path", "corrupted triples path"]
        self.select_node_corrupted = tk.StringVar(
            value=self.select_choices_node_corrupted[0]
        )
        select_menu = tk.OptionMenu(
            panel, self.select_node_corrupted, *self.select_choices_node_corrupted
        )
    ...
    def update(self):
        ...
        tts_files_folder = os.path.join(
            self.controller.ARGS_LIST_GLOBAL[1], ttsConst.TTS_FOLDER_NAME
        )
        ...
        if (
            ...
            or "SplitFrame" in self.controller.selected_frames
        ):
        ...
        if (
            ...
            or "GraphCreationFrame" in self.controller.selected_frames
        ):
    ...
    def next_page(self):
        if not self.train.get() and not self.evaluate.get():
            messagebox.showerror(
                "ERROR", "at least one action (training, evaluation) must be chosen"
            )
            return
        ...
        if self.train.get():
            if not self.train_path.get():
                messagebox.showerror(
                    "ERROR", "For training, please select a training set path"
                )
                return
        if self.evaluate.get():
            ...
            if not self.test_path.get():
                ...
                return
            if (
                self.hits.get() or self.mrr.get()
            ) and not self.nodes_or_corr_path.get():
                messagebox.showerror(
                    "ERROR",
        ...
        self.controller.ARGS_LIST_EVAL.extend(
            ["--model_cls", str(ModelTypes[self.select_model.get()].name)]
        )
        ...
        if not self.train.get():
            ...
            self.controller.ARGS_LIST_EVAL.extend(
                ["--trained_model", self.trained_model_path.get()]
            )
        else:
        ...
        if not self.evaluate.get():
            ...
            ranked_metrics = [
                x.name for x, y in self.rank_metrics_dict.items() if y.get()
            ]
            threshold_metrics = [
                x.name for x, y in self.threshold_metrics_dict.items() if y.get()
            ]
            ...
            if (
                ...
                self.controller.ARGS_LIST_EVAL.extend(
                    [str(x) for x in ast.literal_eval(self.ks.get())]
                )
            # if metrics
            if self.select_node_corrupted.get() == "nodes path":
                self.controller.ARGS_LIST_EVAL.extend(
                    ["--eval_nodes", self.nodes_or_corr_path.get()]
                )
            elif self.select_node_corrupted.get() == "corrupted triples path":
                self.controller.ARGS_LIST_EVAL.extend(
                    ["--corrupted", self.nodes_or_corr_path.get()]
                )
