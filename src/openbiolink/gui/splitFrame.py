...
class SplitFrame(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.title = tk.Label(
            titles_panel, text="(2) Split Creation", font=controller.title_font
        )
        ...
        self.right_rand_box = tk.LabelFrame(
            self.right_box, text="parameter for random split"
        )
        self.rand_box_el = self._create_rand_options_el(self.right_rand_box)
        self.right_time_box = tk.LabelFrame(
            self.right_box, text="parameter for time slice"
        )
    ...
    def _create_rand_options_el(self, parent):
        ...
        test_frac_value = tk.Entry(
            test_frac_frame, textvariable=self.test_frac, width=5
        )
        test_frac_info = tk.Label(
            test_frac_frame, text="as float, e.g. 0.2", font=self.controller.info_font
        )

        # cross val
        self.crossval = tk.BooleanVar(value=False)
        crossval_box = tk.Checkbutton(
            el, text="cross validation", variable=self.crossval
        )
    ...
    def _create_edge_path_el(self, parent):
        ...
        edge_button = tk.Button(
            panel, text="select path ...", command=lambda: self.browse_edge_file()
        )
    ...
    def _create_tn_path_el(self, parent):
        ...
        tn_button = tk.Button(
            panel, text="select path ...", command=lambda: self.browse_tn_file()
        )
        tn_label = tk.Entry(el, textvariable=self.tn_path)
        panel.pack(side="top", fill="x")
        tk.Label(panel, text="true negative edge path:").pack(
    ...
    def _create_nodes_path_el(self, parent):
        ...
        nodes_button = tk.Button(
            panel, text="select path ...", command=lambda: self.browse_nodes_file()
        )
        nodes_label = tk.Entry(el, textvariable=self.nodes_path)
        panel.pack(side="top", fill="x")
        tk.Label(panel, text="nodes file path").pack(
            side="left", anchor="w", padx=5, pady=5
        )
    ...
    def _create_tmo_edge_path_el(self, parent):
        ...
        edge_button = tk.Button(
            panel, text="select path ...", command=lambda: self.browse_tmo_edge_file()
        )
    ...
    def _create_tmo_tn_path_el(self, parent):
        ...
        tn_button = tk.Button(
            panel, text="select path ...", command=lambda: self.browse_tmo_tn_file()
        )
    ...
    def _create_tmo_nodes_path_el(self, parent):
        ...
        nodes_button = tk.Button(
            panel, text="select path ...", command=lambda: self.browse_tmo_nodes_file()
        )
    ...
    def update(self):
        ...
        graph_files_folder = os.path.join(
            self.controller.ARGS_LIST_GLOBAL[1], gcConst.GRAPH_FILES_FOLDER_NAME
        )
    ...
    def next_page(self):
        ...
        self.controller.ARGS_LIST_TRAIN_TEST_SPLIT.extend(
            ["--edges", self.edge_path.get()]
        )
        self.controller.ARGS_LIST_TRAIN_TEST_SPLIT.extend(
            ["--tn_edges", self.tn_path.get()]
        )
        self.controller.ARGS_LIST_TRAIN_TEST_SPLIT.extend(
            ["--nodes", self.nodes_path.get()]
        )

        if self.mode.get() == "time":
            if (
                (self.tmo_edge_path.get() == "")
                or (self.tmo_tn_path.get() == "")
                or (self.tmo_nodes_path.get() == "")
            ):
                messagebox.showerror(
                    "ERROR", "Please provide all three t-minus one paths."
                )
                return
            self.controller.ARGS_LIST_TRAIN_TEST_SPLIT.extend(["--mode", "time"])
            self.controller.ARGS_LIST_TRAIN_TEST_SPLIT.extend(
                ["--tmo_edges", self.tmo_edge_path.get()]
            )
            self.controller.ARGS_LIST_TRAIN_TEST_SPLIT.extend(
                ["--tmo_tn_edges", self.tmo_tn_path.get()]
            )
            self.controller.ARGS_LIST_TRAIN_TEST_SPLIT.extend(
                ["--tmo_nodes", self.tmo_nodes_path.get()]
            )
            ...
            self.controller.ARGS_LIST_TRAIN_TEST_SPLIT.extend(
                ["--test_frac", self.test_frac.get()]
            )

            if self.crossval.get():
                self.controller.ARGS_LIST_TRAIN_TEST_SPLIT.append(
                    "--crossval"
                )  # todo crossval
                self.controller.ARGS_LIST_TRAIN_TEST_SPLIT.extend(
                    ["--val", self.folds.get()]
