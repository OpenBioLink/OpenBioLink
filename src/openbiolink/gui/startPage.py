...
class StartPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        title = tk.Label(
            titles_panel, text="Select Actions", font=controller.title_font
        )
        ...
        g_box = tk.Checkbutton(
            actions_panel, text="(1) Generate Graph", variable=self.g
        )
        self.s = tk.BooleanVar()
        s_box = tk.Checkbutton(
            actions_panel, text="(2) Generate Train Test Split", variable=self.s
        )
        self.e = tk.BooleanVar()
        e_box = tk.Checkbutton(
            actions_panel, text="(3) Apply Testing and Evaluation", variable=self.e
        )
        ...
        actions_panel.pack(
            side="top", fill="both", expand=True, anchor="w", padx=15, pady=10
        )
    ...
    def _create_working_dir_el(self, parent):
        ...
        path_button = tk.Button(
            el, text="select path ...", command=lambda: self.browse_dir()
        )
